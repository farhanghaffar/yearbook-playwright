...
test.describe('Ladder', () => {
    test('Open dropdown popop on clicking manage button', async({page}) => {
        ...
        await page.waitForTimeout(6000); // Temporarily so that test don't fail because of popup not loading on click of manage button
    ...
    test('Toggling Board View ON highlights the Button', async({page}) => {
        ...
        const boardViewBtn = await page.locator('strong + div .cursor-pointer').filter({hasText: 'Board'});
        await expect(boardViewBtn).toBeVisible();
        await boardViewBtn.click();

        const boardViewBtnClassAttribute = await boardViewBtn.getAttribute('class');
        await expect(boardViewBtnClassAttribute).toContain('bg-blue-900');
    ...
    test('Toggling Board View OFF tone-down the Button', async({page}) => {
        ...
        await page.pause();
        const ladderViewBtn = await page.locator('strong + div .cursor-pointer').filter({hasText: 'Ladder'});
        await expect(ladderViewBtn).toBeVisible();

        const boardViewBtnClassAttribute = await boardViewBtn.getAttribute('class');
        if(!boardViewBtnClassAttribute.includes('bg-blue-900')){
            await ladderViewBtn.click();
        }

        await expect(boardViewBtnClassAttribute).not.toContain('bg-blue-900');
